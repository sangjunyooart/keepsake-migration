...
class PiPusher:
    def __init__(self, system_config: dict):
        ...
        self.ssh_user = dist["pi_ssh_user"]
        self.ssh_key = os.path.expanduser(dist["pi_ssh_key_path"])

    def push_adapter(self, lens_name: str, adapter_path: Path, pi_hostname: str) -> dict:
        """
        rsync adapter to Pi, then POST /reload signal.
        Returns {"success": bool, "error": str | None}.
        """
        result = self._rsync(lens_name, adapter_path, pi_hostname)
    ...
    def _rsync(self, lens_name: str, adapter_path: Path, pi_hostname: str) -> dict:
        remote_path = f"/home/pi/keepsake/adapters/{lens_name}/"
        cmd = [
            "rsync", "-avz", "--delete",
            "-e", f"ssh -i {self.ssh_key} -o StrictHostKeyChecking=no -o ConnectTimeout=10",
            f"{adapter_path}/",
            f"{self.ssh_user}@{pi_hostname}:{remote_path}",
        ]
        try:
